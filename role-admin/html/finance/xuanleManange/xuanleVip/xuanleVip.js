layui.config({
    ...
    //----------专享---------
    //base.operateArea('branchCompanyId','b3','c3','areaId','e3','f3','shopId','h3');
    updateLayuiDate('buyTimeStartVip');
    updateLayuiDate('buyTimeEndVip');
    updateLayuiDate('expireTimeStartVip');
    updateLayuiDate('expireTimeEndVip');
    base.multiSelect('downpanel-vip','multSelects1-vip','orderTypes-vip','typesEightVip');
    base.multiSelect('downpanel-vip-status','multSelects1-vip-status','orderTypes-vip-status','typesVipStatus');
    ...
    var tableInsNumVip = table.render({
        elem: '#numVip', limit: 10, cellMinWidth: 80, method: 'get',
        page: {layout: ['count', 'prev', 'page', 'next', 'skip']},
        request: {pageName: "pageNum" ,limitName:"pageSize"},
        url: base.apiUrl() + "/xuanleManager/xuanleFixList?goodsType=1",
        headers:{
        ...
        cols: [[ // 表头
            ...
            {
                field: '', fixed: 'right', align: 'center',title: '操作', templet: function (e) {
                    var str =
    ...
    //查询
    $("#checkVip").on('click',function(){
        var loginName = $("#loginNameVip").val();
        ...
        $("input:checkbox[name='typesVipStatus']:checked").each(function(e) {
    ...
    function exportTable(paramter) {
        base.exportAjax({
            ...
            success:function (res) {
                //excel名
                var name =  "玄乐专享-批量导出-"+new Date().format("yyyyMMddhhmm");
                //excel头部
