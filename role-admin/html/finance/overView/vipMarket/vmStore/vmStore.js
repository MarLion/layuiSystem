layui.config({
    ...
    $(function () {
        base._ajax({
            ...
            data: {
                ...
                viewId: '1',
                parameters: "{'deptId': " + deptId + ",'type': '1'}"
            },
            headers: {
                'xxl_sso_sessionid': window.sessionStorage.getItem('sessionid'),
                'Content-Type': "application/x-www-form-urlencoded; charset=UTF-8",
    ...
    table.on('tool(companyLs)', function (obj) {
        ...
        if (obj.event === 'goPersonalOrder') {
            ...
            var tab_title = name + '个人详情';
            parent.tab.tabAdd({
                href: 'html/finance/overView/vipMarket/vmPersonal/vmPersonal.html?deptId=' + id + '&time=' + time,
                // icon: '',
                title: tab_title
            });
        ...
        }else if (obj.event === 'goStoreCategory') {
            var id = data.id;
            var name = data.deptName;
            var tab_id = 'branchStore1' + id;
            var time = $('#chooseTime').val();
            if (time == "") {
                return layer.msg("请选择日期！");
            }
