layui.config({
    ...
    var companyTable = table.render({
        ...
        where: {
            ...
            viewId: '1',
            parameters: "{'deptId': " + deptId + ", 'searchTime': " + $("#chooseTime").val() + ",'type': '1', 'goodsno': '" + goodsno + "'}"
        },
        method: 'post',
    ...
    $(function () {
        base._ajax({
            ...
            data: {
                ...
                viewId: '1',
                parameters: "{'deptId': " + deptId + ",'type': '1', 'goodsno': '" + goodsno + "'}"
            },
            headers: {
                'xxl_sso_sessionid': window.sessionStorage.getItem('sessionid'),
                'Content-Type': "application/x-www-form-urlencoded; charset=UTF-8",
    ...
    $("#check").on('click', function () {
        ...
        companyTable.reload({
            where: {
                actionName: 'com.xguanjia.client.action.statistics.saleManage.GroupAction$getSaleInfos',
                viewId: '1',
                parameters: "{'deptId': " + deptId + ", 'searchTime': " + time + ",'type': '1', 'goodsno': '" + goodsno + "'}"
            }
        })
    });
    ...
    table.on('tool(companyLs)', function (obj) {
        ...
        if (obj.event === 'goRegionalDetails') {
            ...
            parent.tab.tabAdd({
                href: 'html/finance/overView/vipMarket/vmRegionCategory/vmRegionCategory.html?deptId=' + id + '&time=' + time + '&goodsno=' + goodsno + '&goodsname=' + encodeURI(goodsname),
                // icon: '',
                title: tab_title
            });
            ...
            var goodsno = $("#goodsno").val();
            var goodsname = $("#goodsname").val();
            var tab_id = 'branchReport1' + goodsno + id;
            var tab_title = name + goodsname + '往期销售报表';
            parent.tab.tabAdd({
                id: tab_id,
                href: 'html/finance/overView/vipMarket/vmCategoryHistoryReport/vmCategoryHistoryReport.html?deptId=' + id + '&goodsno=' + goodsno,
                // icon: '',
                title: tab_title
            });
        }
