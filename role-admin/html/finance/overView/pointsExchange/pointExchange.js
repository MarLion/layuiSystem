layui.config({
    ...
    $("#goCompany").on('click', function () {
        ...
        parent.tab.tabAdd({
            href: 'html/finance/overView/pointsExchange/companyDetail/companyDetail.html?time=' + time,
            // icon: '',
            title: '分公司积分兑换详情'
        })
    });
    ...
    function serviceFeeData() {
        ...
        base._ajax({
            url: base.apiOthUrl(),
            data: {
                actionName: 'com.xguanjia.client.action.statistics.saleManage.ServiceChargeAction$getExchangeServiceFeeInfos',
                viewId: '1',
            ...
            success: function (res) {
                ...
                var moneyList = _obj.moneyList;
                var _html = '';
                for (var i=moneyList.length-1;i>=0;i--) {
                    _html +=  '<div class="swiper-slide slide-container">';
                    _html +=    '<p>'+moneyList[i].date+'</p>';
                    _html +=    '<p>积分兑换：</p>';
                    _html +=    '<p>'+moneyList[i].money+'</p>';
                    _html +=  '</div>';
                }
