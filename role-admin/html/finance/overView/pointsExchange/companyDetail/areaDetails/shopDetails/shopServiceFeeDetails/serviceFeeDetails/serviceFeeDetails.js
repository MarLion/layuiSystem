layui.config({
    ...
    var companyTable = table.render({
        ...
        where: {
            actionName: 'com.xguanjia.client.action.statistics.saleManage.ServiceChargeAction$memberList',
            viewId: '1',
            parameters: "{'userId': "+ id +", 'time': " + $("#chooseTime").val() + "}"
        },
        method: 'post',
        ...
        cols: [[
            ...
            {field: 'num', title: '数量'},
            {field: 'waiterMoney', title: '所需服务费'},
            {field: 'userPayWaiterMoney', title: '实付服务费'},
    ...
    $("#check").on('click', function () {
        ...
        companyTable.reload({
            where: {
                actionName: 'com.xguanjia.client.action.statistics.saleManage.ServiceChargeAction$memberList',
                viewId: '1',
                parameters: "{'userId': "+ id +", 'time': " + time + "}"
            }
        })
    ...
    $(function () {
        base._ajax({
            ...
            success: function (res) {
                var _obj = res.data;
                areaName = _obj.shopName;
                $("#shopName").html(_obj.shopName+"服务费记录");
    ...
    table.on('tool(companyPc)', function (obj) {
        ...
        if (obj.event === 'goShopDetails') {
            ...
            parent.tab.tabAdd({
                href: 'html/finance/overView/pointsExchange/personalConsumHistory/personalConsumHistory.html?areaName='
                    +areaName+"&loginName="+loginName+"&name="+name+"&time="+time+"&id="+id,
                // icon: '',
                title: tab_title
            });
        }
