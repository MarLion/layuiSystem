layui.config({
    ...
    var companyTable = table.render({
        ...
        where:{
            ...
            viewId: '1',
            parameters:"{'deptId': " + 1 + ", 'time': " + $("#chooseTime").val() + ", 'type': " + 4 + "}"
        },
        cellMinWidth: 60,
    ...
    table.on('tool(companyLs)', function (obj) {
        ...
        if (obj.event === 'goRegionalDetails') {
            ...
            var tab_title = deptName + '新增设备详情';
            parent.tab.tabAdd({
                href: 'html/finance/overView/peopleEquiAddNum/equiCompanyArea/equiArea.html?time='+ time+'&deptId='+deptId+'&deptName='+encodeURI(deptName),
                // icon: '',
                title: tab_title
            });
