layui.config({
    ...
    var companyTable = table.render({
        ...
        where: {
            ...
            viewId: '1',
            parameters: "{'deptId': '1', 'searchTime': " + $("#chooseTime").val() + ",'type': '0'}"
        },
        method: 'post',
        ...
        cols: [[
            ...
            {field: 'deptName', title: '公司名', templet: '#positionName'},
            ...
            {field: 'right', align: 'center', width: 400, toolbar: '#barDemo', title: '操作'}
        ]],
        skin: 'line', //表格风格
        even: true,
        limit: 15, //每页默认显示的数量
    ...
    $("#check").on('click', function () {
        ...
        companyTable.reload({
            where: {
                actionName: 'com.xguanjia.client.action.statistics.saleManage.GroupAction$getSaleInfos',
                viewId: '1',
                parameters: "{'deptId': '1', 'searchTime': " + time + ",'type': '0'}"
            }
        })
    });
    ...
    table.on('tool(companyLs)', function (obj) {
        ...
        if (obj.event === 'goRegionalDetails') {
            ...
            parent.tab.tabAdd({
                href: 'html/finance/overView/lsShoppingSale/isRegion/isRegion.html?deptId=' + id + '&time=' + time,
                // icon: '',
                title: tab_title
            });
            ...
            parent.tab.tabAdd({
                href: 'html/finance/overView/lsShoppingSale/isHistoryReport/isHistoryReport.html?deptId=' + id,
                // icon: '',
                title: tab_title
            });
        }else if (obj.event === 'goCompanyCategory') {
            var id = data.id;
            var name = data.deptName;
            var tab_id = 'branchCategorya1' + id;
            var time = $('#chooseTime').val();
            if (time == "") {
                return layer.msg("请选择日期！");
            }
            var tab_title = name + '垄上扶贫品类销售详情';
            parent.tab.tabAdd({
                id: tab_id,
                href: 'html/finance/overView/lsShoppingSale/lsCategory/lsCategory.html?deptId='+id+'&time=' + time + '&name=' + encodeURI(name),
                // icon: '',
                title: tab_title
            });
        }
