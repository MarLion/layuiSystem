layui.config({
    ...
    var areaTable = table.render({
        ...
        where:{
            ...
            parameters:'{"time":'+time+',"deptId":'+deptId+',"type":5}',
        },
    ...
    $("#check").on('click',function () {
        ...
        areaTable.reload({
            where:{
                actionName:'com.xguanjia.client.action.statistics.memberstatistics.MemberstatisticsAction$getSaleInfos',
                parameters:'{"time":'+$("#chooseTime").val()+',"deptId":'+deptId+',"type":5}',
            },
        })
    });
    ...
    table.on('tool(areaLs)', function (obj) {
        ...
        if (obj.event === 'goRegionalDetails') {
            ...
            var tab_title = shopName + '新增会员详情';
            parent.tab.tabAdd({
                href: 'html/finance/overView/peopleVipAddNum/ngCompanyAreaShop/ngArea.html?time='+ time+'&deptId='+deptId+'&deptName='+encodeURI(shopName),
                title: tab_title
            });
        } else if (obj.event === 'goHistoryReport') {
            var tab_title = shopName + '新增会员报表';
            parent.tab.tabAdd({
                href: 'html/finance/overView/peopleVipAddNum/ngHistoryReport/ngHistoryReport.html?deptId='+ deptId+'&deptName='+encodeURI(shopName),
                title: tab_title
            });
        }
    });
