layui.config({
    ...
    var companyTable = table.render({
        ...
        where: {
            ...
            viewId: '1',
            parameters: "{'deptId':"+deptId+",'searchTime':'" + $("#chooseTime").val() + "','goodsno':'"+goodsno+"'}"
        },
        method: 'post',
    ...
    table.on('tool(companyPc)', function (obj) {
        ...
        if (obj.event === 'goHistoryReport') {
            ...
            var tab_title = deptName +goodsname+ '销售详情';
            parent.tab.tabAdd({
                href: 'html/finance/overView/healthProduct/hpPinLeiDetails/shopSaleDetails/sSaleDetails.html?id='+id+"&time="+time+"&goodsno="+goodsno,
                // icon: '',
                title: tab_title
            });
        }
