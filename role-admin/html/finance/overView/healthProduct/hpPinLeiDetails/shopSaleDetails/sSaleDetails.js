layui.config({
    ...
    var companyTable = table.render({
        ...
        where: {
            actionName: 'com.xguanjia.client.action.statistics.saleManage.ProductStaAction$getProductShopOrder',
            viewId: '1',
            parameters: "{'deptId':"+ deptId +",'searchTime':'" + time + "','goodsno':'"+goodsno+"'}"
        },
        method: 'post',
    ...
    table.on('tool(companyPc)', function (obj) {
        ...
        if (obj.event === 'goShopDetails') {
            ...
            var tab_title = '会员消费订单';
            parent.tab.tabAdd({
                href: 'html/finance/overView/healthProduct/hpPinLeiDetails/memberComsumeOrder/mcOrder.html?' +
                    'goodsno='+goodsno+"&time="+time+"&id="+id+"&address="+address,
                // icon: '',
                title: tab_title
            });
        }
