layui.config({
    ...
    if (v != undefined) {
        time = v['time'];
        goodsno = v['goodsno'];
        goodsname = decodeURI(v['goodsname']);
        $("#chooseTime").val(time);
    ...
    var companyTable = table.render({
        ...
        where: {
            ...
            viewId: '1',
            parameters: "{'deptId':1,'searchTime':'" + $("#chooseTime").val() + "','goodsno':'"+goodsno+"'}"
        },
        method: 'post',
    ...
    $(function () {
        base._ajax({
            ...
            data: {
                ...
                viewId: '1',
                parameters: "{'deptId':1,'goodsno':'"+goodsno+"'}"
            },
            headers: {
                'xxl_sso_sessionid': window.sessionStorage.getItem('sessionid'),
                'Content-Type': "application/x-www-form-urlencoded; charset=UTF-8",
    ...
    table.on('tool(companyPc)', function (obj) {
        ...
        if (obj.event === 'goHistoryReport') {
            ...
            var tab_title = deptName +goodsname+ '往期销售报表';
            parent.tab.tabAdd({
                href: 'html/finance/overView/healthProduct/hpPinLeiDetails/productHistoryReport/phReport.html?deptId='+id+'&goodsno='+goodsno,
                // icon: '',
                title: tab_title
            });
            ...
            parent.tab.tabAdd({
                href: 'html/finance/overView/healthProduct/hpPinLeiDetails/subCompanySaleDetails/scSaleDetails.html?' +
                    'deptId='+id+"&goodsno="+goodsno+"&goodsname="+goodsname+"&time="+time,
                // icon: '',
                title: tab_title
            });
        }
