layui.config({
    ...
    var companyTable = table.render({
        ...
        where: {
            actionName: 'com.xguanjia.client.action.statistics.saleManage.ProductStaAction$getSaleInfosByGoods',
            viewId: '1',
    ...
    table.on('tool(companyPc)', function (obj) {
        var data = obj.data;
        if (obj.event === 'goSubCompanyDetails') {
            var id = data.id;
            ...
            var tab_title = goodsname + '往期销售报表';
            parent.tab.tabAdd({
                href: 'html/finance/overView/healthProduct/hpPinLeiDetails/productHistoryReport/phReport.html?deptId='+deptId+'&goodsno='+goodsno,
                // icon: '',
                title: tab_title
            });
        }
