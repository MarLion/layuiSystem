layui.config({
    ...
    var areaTable = table.render({
        ...
        where:{
            ...
            parameters:'{"time":'+time+',"deptId":'+deptId+',"type":3}',
        },
        ...
        cols: [[
            ...
            {field: 'daynum', align: 'center',title: '登陆人数'},
    ...
    $("#check").on('click',function () {
        ...
        areaTable.reload({
            where:{
                actionName:'com.xguanjia.client.action.statistics.memberstatistics.MemberstatisticsAction$getSaleInfos',
                parameters:'{"time":'+$("#chooseTime").val()+',"deptId":'+deptId+',"type":3}',
            },
        })
    });
    ...
    table.on('tool(areaLs)', function (obj) {
        ...
        if (obj.event === 'goRegionalDetails') {
            ...
            var tab_title = shopName + '会员登陆详情';
            parent.tab.tabAdd({
                href: 'html/finance/overView/peopleLoginNum/ngCompanyArea/ngArea.html?time='+ time+'&deptId='+deptId+'&deptName='+encodeURI(shopName),
                title: tab_title
            });
        } else if (obj.event === 'goHistoryReport') {
            var tab_title = shopName + '登陆人数报表';
            parent.tab.tabAdd({
                href: 'html/finance/overView/peopleLoginNum/ngHistoryReport/ngHistoryReport.html?deptId='+ deptId+'&deptName='+encodeURI(shopName),
                title: tab_title
            });
        }
    });
