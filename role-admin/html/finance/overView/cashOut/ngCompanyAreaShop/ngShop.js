layui.config({
    ...
    if (v != undefined) {
        deptId = v['deptId'];
        deptName = v['deptName'];
        $("#companyTitle").html(decodeURI(deptName)+'提现销售情况');
        time = v['time'];
        $("#chooseTime").val(time);
    ...
    var shopTable = table.render({
        ...
        where: {
            actionName: 'com.xguanjia.client.action.statistics.saleManage.DrawCashAction$getMoneyInfo',
            viewId: '1',
            parameters: "{'deptId': " + deptId + ", 'searchTime': " + $("#chooseTime").val() + "}"
        },
        method: 'post',
        cellMinWidth: 60,
        ...
        cols: [[
            ...
            {field: 'deptName', title: '门店名'},
            {field: 'totalMoney', title: '提现额'},
            {field: 'avgWeekMoney', title: '周平均提现额'},
            {field: 'avgMonthMoney', title: '月平均提现额'},
            {field: 'dayRise', title: '同比昨日增长', templet: '#dayRise',templet:function (d) {
                    var obj = changColor(d.dayGrowth);
                    var _html = '<span>'+d.dayRise+'</span><span class="ml10"><img src="'+obj.src+'"/></span><span class="ml10 '+obj.class+'">'+d.dayGrowth+'</span>';
                    return _html;
                }},
    ...
    $(function(){
        base._ajax({
            url: base.apiOthUrl(),
            data: {
                actionName: 'com.xguanjia.client.action.statistics.saleManage.DrawCashAction$getWeekData',
                viewId: '1',
    ...
    table.on('tool(shopLs)', function (obj) {
        ...
        if (obj.event === 'goMemberDetails') {
            ...
            var tab_title = deptName + '提现个人情况';
            parent.tab.tabAdd({
                href: 'html/finance/overView/cashOut/ngCompanyAreaShopMember/ngMember.html?time='+ time+'&deptId='+deptId+'&deptName='+encodeURI(deptName),
                // icon: '',
                title: tab_title
            });
            ...
            var deptId = data.id;
            var deptName =data.deptName;
            var tab_title = deptName + '往期提现报表';
            parent.tab.tabAdd({
                href: 'html/finance/overView/cashOut/ngHistoryReport/ngHistoryReport.html?deptId='+ deptId+'&deptName='+encodeURI(deptName),
                // icon: '',
                title: tab_title
            });
        }
