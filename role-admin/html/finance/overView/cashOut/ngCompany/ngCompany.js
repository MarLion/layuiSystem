layui.config({
    ...
    var companyTable = table.render({
        ...
        where: {
            ...
            viewId: '1',
            parameters: "{'deptId': " + 1 + ", 'searchTime': " + $("#chooseTime").val() + "}"
        },
        method: 'post',
        cellMinWidth: 60,
    ...
    table.on('tool(companyLs)', function (obj) {
        ...
        if (obj.event === 'goRegionalDetails') {
            ...
            var tab_title = deptName + '提现区域详情';
            parent.tab.tabAdd({
                href: 'html/finance/overView/cashOut/ngCompanyArea/ngArea.html?time='+ time+'&deptId='+deptId+'&deptName='+encodeURI(deptName),
                // icon: '',
                title: tab_title
            });
