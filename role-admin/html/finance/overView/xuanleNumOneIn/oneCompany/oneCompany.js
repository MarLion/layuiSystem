layui.config({
    ...
    var companyTable = table.render({
        ...
        where:{
            ...
            viewId: '1',
            parameters:"{'deptId': " + 1 + ", 'time': " + $("#chooseTime").val() + ", 'type': " + 8 + "}"
        },
        cellMinWidth: 60,
        ...
        cols: [[
            ...
            {field: 'daynum',align: 'center', title: '玄乐1号转入总额'},
    ...
    $("#check").on('click',function () {
        ...
        companyTable.reload({
            where:{
                actionName:'com.xguanjia.client.action.statistics.steadilyhigh.SteadilyhighAction$getSaleInfos',
                viewId: '1',
                parameters:'{"time":'+$("#chooseTime").val()+',"deptId":1,"type":8}',
            },
        })
    });
    ...
    table.on('tool(companyLs)', function (obj) {
        ...
        if (obj.event === 'goRegionalDetails') {
            ...
            var tab_title = deptName + '玄乐1号转入';
            parent.tab.tabAdd({
                href: 'html/finance/overView/xuanleNumOneIn/oneCompanyArea/oneArea.html?time='+ time+'&deptId='+deptId+'&deptName='+encodeURI(deptName),
                // icon: '',
                title: tab_title
            });
            ...
            var deptId = data.shopId;
            var deptName =data.shopName;
            var tab_title = deptName + '历史报表';
            parent.tab.tabAdd({
                href: 'html/finance/overView/xuanleNumOneIn/oneHistoryReport/oneHistoryReport.html?deptId='+ deptId+'&deptName='+encodeURI(deptName),
                // icon: '',
                title: tab_title
            });
        }
