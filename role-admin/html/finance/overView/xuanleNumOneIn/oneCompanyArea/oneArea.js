layui.config({
    ...
    if (v != undefined) {
        deptId = v['deptId'];
        deptName = v['deptName'];
        $("#companyTitle").html(decodeURI(deptName)+'玄乐1号转入情况');
        time = v['time'];
        $("#chooseTime").val(time);
    ...
    var areaTable = table.render({
        ...
        where:{
            ...
            viewId: '1',
            parameters:"{'deptId': " + deptId + ", 'time': " + $("#chooseTime").val() + ", 'type': " + 8 + "}"
        },
        cellMinWidth: 60,
    ...
    $(function(){
        base._ajax({
            ...
            data: {
                ...
                viewId: '1',
                parameters: "{'deptId': " + deptId + ", 'type': " + 8 + "}"
            },
            headers: {
                'xxl_sso_sessionid': window.sessionStorage.getItem('sessionid'),
                'Content-Type': "application/x-www-form-urlencoded; charset=UTF-8",
    ...
    table.on('tool(areaLs)', function (obj) {
        ...
        if (obj.event === 'goShopDetails') {
            ...
            parent.tab.tabAdd({
                href: 'html/finance/overView/xuanleNumOneIn/oneCompanyAreaShop/oneShop.html?time='+ time+'&deptId='+deptId+'&deptName='+encodeURI(deptName),
                // icon: '',
                title: tab_title
            });
            ...
            var deptId = data.shopId;
            var deptName =data.shopName;
            var tab_title = deptName + '往期玄乐1号转入报表';
            parent.tab.tabAdd({
