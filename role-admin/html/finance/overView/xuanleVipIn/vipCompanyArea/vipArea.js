layui.config({
    ...
    if (v != undefined) {
        deptId = v['deptId'];
        deptName = v['deptName'];
        $("#companyTitle").html(decodeURI(deptName)+'玄乐专享转入情况');
        time = v['time'];
        $("#chooseTime").val(time);
    ...
    var areaTable = table.render({
        ...
        where:{
            ...
            viewId: '1',
            parameters:"{'deptId': " + deptId + ", 'time': " + $("#chooseTime").val() + ", 'type': " + 14 + "}"
        },
        cellMinWidth: 60,
    ...
    $(function(){
        base._ajax({
            ...
            data: {
                ...
                viewId: '1',
                parameters: "{'deptId': " + deptId + ", 'type': " + 14 + "}"
            },
            headers: {
                'xxl_sso_sessionid': window.sessionStorage.getItem('sessionid'),
                'Content-Type': "application/x-www-form-urlencoded; charset=UTF-8",
    ...
    table.on('tool(areaLs)', function (obj) {
        ...
        if (obj.event === 'goShopDetails') {
            ...
            parent.tab.tabAdd({
                href: 'html/finance/overView/xuanleVipIn/vipCompanyAreaShop/vipShop.html?time='+ time+'&deptId='+deptId+'&deptName='+encodeURI(deptName),
                // icon: '',
                title: tab_title
            });
