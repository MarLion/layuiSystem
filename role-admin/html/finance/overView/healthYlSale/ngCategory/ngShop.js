layui.config({
    ...
    var companyTable = table.render({
        elem: '#companyLs',
        url: base.apiUrl() + '/health/getSaleInfos',
        headers: {
    ...
    $(function () {
        base._ajax({
            url: base.apiUrl() + '/health/getWeekData',
            data: {
                deptId: deptId
            },
    ...
    table.on('tool(companyLs)', function (obj) {
        ...
        if (obj.event === 'goUserDetails') {
            ...
            var tab_title = decodeURI(goodsName) + "-" + deptName + '-门店详情';
            parent.tab.tabAdd({
                id: tab_id,
                href: 'html/finance/overView/healthYlSale/ngCategory/ngPerson.html?time=' + time + '&shopId=' + deptId + '&shopName=' + decodeURI(deptName) + '&goodsno=' + decodeURI(goodsno) + '&goodsName=' + decodeURI(goodsName),
                // icon: '',
                title: tab_title
            });
        } else if (obj.event === 'goHistoryReport') {
            var id = data.id;
            var name = decodeURI(data.deptName);
            var tab_title = decodeURI(goodsName) + "-" + name + '历史销售报表';
            parent.tab.tabAdd({
                id: tab_id,
                href: 'html/finance/overView/healthYlSale/ngHistoryReport/ngHistoryReport.html?deptId=' + id + '&deptName=' + decodeURI(name) + '&goodsno=' + decodeURI(goodsno) + '&goodsName=' + decodeURI(goodsName),
                // icon: '',
                title: tab_title
            });
        }
