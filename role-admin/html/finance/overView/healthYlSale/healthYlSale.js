layui.config({
    ...
    $(function() {
        if(roleLevel==2){
            $("#goCompany").html('区域详情');
            $("#goCategory").html('健康医疗品类区域详情');

        }else if(roleLevel==3){
            $("#goCompany").html('门店详情');
            $("#goCategory").html('健康医疗品类门店详情');

        }else{
            $("#goCompany").html('分公司详情');
            $("#goCategory").html('健康医疗品类分公司详情');
        }
    });
    $("#goCategory").on('click',function () {
        ...
        var href='';
        if(roleLevel==2){
            href='html/finance/overView/healthYlSale/ngCategory/ngCategory.html?time='+ time+'&deptId=-1';
        }else if(roleLevel==3){
            href='html/finance/overView/healthYlSale/ngCategory/ngCategory.html?time='+ time+'&deptId=-1';
        }else{
            href='html/finance/overView/healthYlSale/ngCategory/ngCategory.html?time='+ time+'&deptId=1';
    ...
    $("#goCompany").on('click',function () {
        ...
        if(roleLevel==2){
            title='健康医疗区域销售详情';
            href='html/finance/overView/healthYlSale/ngCompany/ngArea.html?time='+ time+'&deptId=-1';
        }else if(roleLevel==3){
            title='健康医疗门店销售详情';
            href='html/finance/overView/healthYlSale/ngCompany/ngShop.html?time='+ time+'&deptId=-1';
        }else{
            title='健康医疗分公司销售详情';
            href='html/finance/overView/healthYlSale/ngCompany/ngCompany.html?time='+ time;
        }
        parent.tab.tabAdd({
            href: href,
            // icon: '',
    ...
    function ridgePovertyData(){
        ...
        base._ajax({
            url:base.apiUrl()+'/health/getSaleTotalInfos',
            data:{
                searchTime:time,
            },
    ...
    function ridgePovertyEcharts() {
        var m = base.show_load_layer(); //loading层
        base._ajax({
            url:base.apiUrl()+'/health/getChartData',
            data:{
                deptId:'1'
            },
            success:function (res) {
    ...
    function ridgePovertyEchartsSearch() {
        ...
        var m = base.show_load_layer(); //loading层
        base._ajax({
            url:base.apiUrl()+'/health/getChartDataByData',
            data:{
                deptId:'1',
                startTime:begindate,
                endTime:enddate,
