layui.config({
    ...
    $("#goCompany").on('click',function () {
        ...
        if(roleLevel==2){
            title='全球代购区域销售详情';
            href='html/finance/overView/globalShopSale/ngCompanyArea/ngArea.html?time='+ time+'&deptId=-1';
        }else if(roleLevel==3){
            title='全球代购门店销售详情';
            href='html/finance/overView/globalShopSale/ngCompanyAreaShop/ngShop.html?time='+ time+'&deptId=-1';
        }else{
            title='全球代购分公司销售详情';
            href='html/finance/overView/globalShopSale/ngCompany/ngCompany.html?time='+ time;
        }
        parent.tab.tabAdd({
            href: href,
            // icon: '',
    ...
    function ridgePovertyData(){
        ...
        base._ajax({
            url:base.apiUrl()+'/shop/getSaleTotalInfos',
            data:{
                searchTime:time,
            },
    ...
    function ridgePovertyEcharts() {
        var m = base.show_load_layer(); //loading层
        base._ajax({
            url:base.apiUrl()+'/shop/getChartData',
            data:{
                deptId:'1'
            },
            success:function (res) {
    ...
    function ridgePovertyEchartsSearch() {
        ...
        var m = base.show_load_layer(); //loading层
        base._ajax({
            url:base.apiUrl()+'/shop/getChartDataByData',
            data:{
                deptId:'1',
                startTime:begindate,
                endTime:enddate,
