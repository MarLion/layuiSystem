layui.config({
    ...
    var companyTable = table.render({
        ...
        cols: [[
            ...
            {field: 'right', align: 'center',width:260, toolbar: '#barDemo', title: '操作'}
        ]],
        skin: 'line', //表格风格
        even: true,
        //数据回调
    ...
    table.on('tool(companyLs)', function (obj) {
        ...
        if (obj.event === 'goRegionalDetails') {
            ...
            var tab_title = deptName + '充值区域详情';
            parent.tab.tabAdd({
                href: 'html/finance/overView/recharge/ngCompanyArea/ngArea.html?time='+ time+'&deptId='+deptId+'&deptName='+encodeURI(deptName),
                // icon: '',
                title: tab_title
            });
