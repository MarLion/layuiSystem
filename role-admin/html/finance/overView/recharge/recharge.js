layui.config({
    ...
    $(function() {
        if(roleLevel==2){
            $("#goCompany").html('区域充值情况');
        }else if(roleLevel==3){
            $("#goCompany").html('门店充值情况');
        }else{
            $("#goCompany").html('分公司充值情况');
        }
    });
    ...
    $("#goCompany").on('click',function () {
        ...
        var title = "";
        if (roleLevel == 2) {
            href = 'html/finance/overView/recharge/ngCompanyArea/ngArea.html?deptId=-1&time=' + time;
            title = '区域充值情况';
        }else if (roleLevel == 3) {
            href = 'html/finance/overView/recharge/ngCompanyAreaShop/ngShop.html?deptId=-1&time=' + time;
            title = '门店充值情况';
        }else {//0、1
            href = 'html/finance/overView/recharge/ngCompany/ngCompany.html?time=' + time;
            title = '分公司充值情况';
        }
        parent.tab.tabAdd({
            href: href,
    ...
    function ridgePovertyData(){
        ...
        base._ajax({
            url: base.apiOthUrl(),
            data: {
                actionName: 'com.xguanjia.client.action.statistics.saleManage.RechargeAction$getRechargeInfos',
                viewId: '1',
            ...
            success:function (res) {
                var _obj = res.data;
                $("#saleMoney").html(_obj.totalMoney);
                $("#weekSaleMoney").html(_obj.avgWeekMoney);
                $("#monthSaleMoney").html(_obj.avgMonthMoney);
                document.getElementById("todayRise").innerHTML= _obj.dayRise;
                changeColor(_obj.dayRises,'today-per-icon','todayPercent');
                document.getElementById("todayPercent").innerHTML= _obj.dayRises;
                document.getElementById("monthRise").innerHTML= _obj.monthRise;
                changeColor(_obj.monthRises,'month-per-icon','monthPercent');
                document.getElementById("monthPercent").innerHTML= _obj.monthRises;
    ...
    function ridgePovertyEcharts() {
        ...
        base._ajax({
            url: base.apiOthUrl(),
            data: {
                actionName: 'com.xguanjia.client.action.statistics.saleManage.RechargeAction$searchOrder',
                viewId: '1',
                parameters: "{'deptId': " + 1 + " }"
            },
            headers: {
