layui.config({
    ...
    var companyTable = table.render({
        ...
        where:{
            ...
            parameters:"{'time': "+ time +",'integtalType':"+integtalType+",'shopId':"+1+"}"
        },
        ...
        cols:[[
            ...
            , {field: 'name', align:'center',title: '公司名'}
            , {field: 'integral', align:'center',title: '今日'+titleName+'数'}
            , {field: 'yestIntegral',align:'center', title: '昨日'+titleName+'数'}
            , {field: 'dayRise', align:'center',title: '同比昨日'+titleName1,templet:function (d) {
            ...
            , {field: 'monthRise', align:'center',title: '同比月平均'+titleName1,templet:function (d) {
            ...
            ,{field:'status',title:'操作',align:'center',width:320,templet: function (e) {
                var  str ='  <a class="layui-btn layui-btn-normal   layui-btn-xs" src="javaScript:void(0)" lay-event="branchCompanyDetail">区域</a>'
                return str;
            }}
        ]],
    ...
    table.on('tool(companyLs)', function (obj) {
        ...
        var name = prefixName+deptName+"所属区域积分详情";
        var id =integtalType+shopId;
        //查看分公司详情
        if (layEvent === 'branchCompanyDetail') {
            parent.tab.tabAdd({

                href: 'html/finance/overView/platPointsCompanyAreaDetail/platPointsCompanyAreaDetail.html?time='+time+'&shopId='+shopId+"&integtalType="+integtalType,
                // icon: '',
                id:id,
                title: name
            })
