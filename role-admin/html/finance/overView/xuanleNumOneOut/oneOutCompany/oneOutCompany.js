layui.config({
    ...
    var companyTable = table.render({
        ...
        where:{
            ...
            viewId: '1',
            parameters:"{'deptId': " + 1 + ", 'time': " + $("#chooseTime").val() + ", 'type': " + 9 + "}"
        },
        cellMinWidth: 60,
        ...
        cols: [[
            ...
            {field: 'shopName', align: 'center',title: '公司名'},
            {field: 'daynum',align: 'center', title: '玄乐1号转出总额'},
            {field: 'weekMean',align: 'center', title: '本周日平均金额'},
            {field: 'monthMean',align: 'center', title: '本月日平均金额'},
    ...
    table.on('tool(companyLs)', function (obj) {
        ...
        if (obj.event === 'goRegionalDetails') {
            ...
            var tab_title = deptName + '玄乐1号转出';
            parent.tab.tabAdd({
                href: 'html/finance/overView/xuanleNumOneOut/oneOutCompanyArea/oneOutArea.html?time='+ time+'&deptId='+deptId+'&deptName='+encodeURI(deptName),
                // icon: '',
                title: tab_title
            });
            ...
            var deptId = data.shopId;
            var deptName =data.shopName;
            var tab_title = deptName + '往期玄乐1号转出报表';
            parent.tab.tabAdd({
                href: 'html/finance/overView/xuanleNumOneOut/oneOutHistoryReport/oneOutHistoryReport.html?deptId='+ deptId+'&deptName='+encodeURI(deptName),
                // icon: '',
                title: tab_title
            });
        }
