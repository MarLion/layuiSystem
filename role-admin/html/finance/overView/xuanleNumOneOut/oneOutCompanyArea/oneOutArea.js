layui.config({
    ...
    if (v != undefined) {
        deptId = v['deptId'];
        deptName = v['deptName'];
        $("#companyTitle").html(decodeURI(deptName)+'玄乐1号转出情况');
        time = v['time'];
        $("#chooseTime").val(time);
    ...
    var areaTable = table.render({
        ...
        where:{
            ...
            viewId: '1',
            parameters:"{'deptId': " + deptId + ", 'time': " + $("#chooseTime").val() + ", 'type': " + 9 + "}"
        },
        cellMinWidth: 60,
        ...
        cols: [[
            ...
            {field: 'daynum', title: '玄乐1号转出总额'},
            {field: 'weekMean', title: '周日平均金额'},
            {field: 'monthMean', title: '月日平均金额'},
    ...
    $(function(){
        base._ajax({
            ...
            data: {
                ...
                viewId: '1',
                parameters: "{'deptId': " + deptId + ", 'type': " + 9 + "}"
            },
            headers: {
                'xxl_sso_sessionid': window.sessionStorage.getItem('sessionid'),
                'Content-Type': "application/x-www-form-urlencoded; charset=UTF-8",
    ...
    table.on('tool(areaLs)', function (obj) {
        ...
        if (obj.event === 'goShopDetails') {
            ...
            parent.tab.tabAdd({
                href: 'html/finance/overView/xuanleNumOneOut/oneOutCompanyAreaShop/oneOutShop.html?time='+ time+'&deptId='+deptId+'&deptName='+encodeURI(deptName),
                // icon: '',
                title: tab_title
            });
