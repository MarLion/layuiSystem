layui.config({
    ...
    var areaTable = table.render({
        ...
        where:{
            ...
            parameters:'{"time":'+time+',"deptId":'+deptId+',"type":2}',
        },
        ...
        cols: [[
            ...
            {field: 'shopName', align: 'center',title: '部门名'},
            {field: 'daynum',align: 'center', title: '签到人数'},
            {field: 'weekMean', align: 'center',title: '本周日平均数'},
            {field: 'monthMean',align: 'center', title: '本月日平均数'},
            {field: 'day_Rise',align: 'center', title: '同比昨日增长', templet:function (d) {
                    var obj = changColor(d.dayRise);
                    var _html = '<span>'+d.dayRise+'</span><span class="ml10"><img src="'+obj.src+'"/></span><span class="ml10 '+obj.class+'">'+d.dayRise+'</span>';
                    return _html;
                }},
            {field: 'monthRise', align: 'center',title: '同比月平均增长', templet:function (d) {
    ...
    $("#check").on('click',function () {
        ...
        areaTable.reload({
            where:{
                actionName:'com.xguanjia.client.action.statistics.memberstatistics.MemberstatisticsAction$getSaleInfos',
                parameters:'{"time":'+$("#chooseTime").val()+',"deptId":'+deptId+',"type":2}',
            },
        })
    });
    ...
    table.on('tool(areaLs)', function (obj) {
        ...
        if (obj.event === 'goRegionalDetails') {
            ...
            var tab_title = shopName + '会员签到详情';
            parent.tab.tabAdd({
                href: 'html/finance/overView/peopleSignNum/ngCompanyAreaShop/ngArea.html?time='+ time+'&deptId='+deptId+'&deptName='+encodeURI(shopName),
                title: tab_title
            });
        } else if (obj.event === 'goHistoryReport') {
            var tab_title = shopName + '签到人数报表';
            parent.tab.tabAdd({
                href: 'html/finance/overView/peopleSignNum/ngHistoryReport/ngHistoryReport.html?deptId='+ deptId+'&deptName='+encodeURI(shopName),
                title: tab_title
            });
        }
    });
